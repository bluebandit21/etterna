...
{
	const auto fMusicRate = GAMESTATE->m_SongOptions.GetCurrent().m_fMusicRate;
	const auto fHitUpdateDelta =
	  (GAMESTATE->m_Position.m_LastBeatUpdate - RageTimer(tm)) -
	  padStickSeconds;
	const auto fMusicSeconds =
	  GAMESTATE->m_Position.m_fMusicSeconds - (fHitUpdateDelta * fMusicRate);
	const auto fSongBeat = GAMESTATE->m_pCurSteps
							 ? m_Timing->GetBeatFromElapsedTime(fMusicSeconds)
	...
	if (iRowOfOverlappingNoteOrRow != -1 && col != -1) {
		...
		m_pPlayerStageStats->InputData.emplace_back(
		  !bRelease, col, fMusicSeconds, iRowOfOverlappingNoteOrRow, 0.F);
	}
