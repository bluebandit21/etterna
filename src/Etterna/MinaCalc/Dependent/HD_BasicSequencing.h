...
/// basic pattern permutations are based on two successive non-empty col types
/// as noted above. this is important, for basic pattern structure we do not
/// care about any notes on the other hand, we only care about sequences of
/// notes on this hand, this means that no sequencing done by the row by row
/// sequencer should take place if col == col_empty (nothing on this hand)
enum base_type
{
	base_left_right,
	base_right_left,
...
/// note, base_left_right and base_right_left will be referred to as cross
/// column hits, as in, successive single notes that cross columns and are
/// exclusive of ohjumps
inline auto
