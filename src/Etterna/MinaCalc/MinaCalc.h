 ...
 * memory allocations. Almost every file that exists (except a handful) fit
 * this.
 */
...
class Calc
{
  ...
  private:
	 ...
	 * the chisel functions. Hardcode a limit for nps (100) and if we hit it
	 * just return max value for the calc and move on, there's no point in
  ...
  public:
	 ...
	 */
	std::array<std::array<std::vector<float>, NUM_CalcPatternMod>, num_hands>
	  pmod_vals{};

	/** Holds base calculated difficulties, one float per interval per type per
	 * hand. Contains NPS, MSD, Runningman, and Tech related values. Mostly used
	 * for its NPSBase values.
	 */
	std::array<std::array<std::vector<float>, NUM_CalcDiffValue>, num_hands>
	  init_base_diff_vals{};
	 ...
	 * that pattern mods that push down to extreme degrees stuff, like
	 * jumptrills or roll walls, will also implicitly push down their effect on
	 * stam to to the point where it may be considered a "break", even though
	 * it's really not. At best it's a very different kind of stamina drain
	 ...
	 */
	std::array<std::array<std::vector<float>, NUM_Skillset>, num_hands>
	  base_adj_diff{};
	...
	/// Total number of intervals for the current file/rate (one per half
	/// second)
	int numitv = 0;
	...
	void resize_interval_dependent_vectors(size_t amt)
	{
		...
		// grow each vector
		// resize is used to construct defaults, the space should be used
		// immediately
		adj_ni.resize(amt);
		itv_size.resize(amt);
