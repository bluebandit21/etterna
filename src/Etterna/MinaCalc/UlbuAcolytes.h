...
inline void
Smooth(std::vector<float>& input, const float neutral, const int end_interval)
{
	auto f2 = neutral;
	auto f3 = neutral;
...
inline void
MSSmooth(std::vector<float>& input, const float neutral, const int end_interval)
{
	auto f2 = neutral;
...
/// checks to see if the noteinfo will fit in our static arrays, if it won't
/// it's some garbage joke file and we can throw it out, setting values to 0
inline auto
...
{
	// an inf rowtime means 0 bpm or some other odd gimmick that may break
	// things skip this file nan/inf can occur before the end of the file but
	// the way these are generated, the last should be the largest therefore if
	// any are inf, this is inf
