...
Steps::TidyUpData()
{
	...
	if (m_StepsType == StepsType_Invalid) {
		Locator::getLogger()->debug(
		  "Detected steps with unknown style '{}' in '{}'",
	...
	if (!m_sFilename.empty() && m_sNoteDataCompressed.empty()) {
		...
		if (!this->GetNoteDataFromSimfile()) {
			Locator::getLogger()->warn(
			  "Couldn't load the {} chart's NoteData from \"{}\"",
...
Steps::CalcEtternaMetadata(Calc* calc)
{
	...
	if (m_StepsType != StepsType_dance_single) {
		int columnCount = GAMEMAN->GetStepsTypeInfo(m_StepsType).iNumTracks;
	...
	static auto GetCalcDebugJack(T* p, lua_State* L) -> int
	{
		...
		for (auto hand = 0; hand < 2; hand++) {
			...
			for (size_t i = 0; i < vals.size(); i++) {
				...
				auto v3 = 0.F;
				auto v4 = loss_vals[i];
				// this is required because stam_vals is not guaranteed the same
				// size also due to a calc bug
				if (i < stam_vals.size())
					v3 = stam_vals[i];
