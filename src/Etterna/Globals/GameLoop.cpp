...
CheckGameLoopTimerSkips(float fDeltaTime)
{
	...
	if (fabsf(fDifference) > 0.002f && fabsf(fDifference) < 0.100f)
		Locator::getLogger()->trace("GameLoop timer skip: {} FPS, expected "
									"{:.3f}, got {:.3f} ({:.3f} difference)",
									iThisFPS,
									fExpectedTime,
									fDeltaTime,
									fDifference);
...
CheckFocus()
{
	...
	// This helps to mitigate game stutter caused by CPU scheduling between
	// frames
	if (hasFocus)
...
void
setGameFocused(bool isFocused)
{
	if (isFocused == hasFocus)
		return;
	hasFocus = isFocused;

	Locator::getLogger()->trace("App {} focus",
								isFocused ? "has" : "doesn't have");
