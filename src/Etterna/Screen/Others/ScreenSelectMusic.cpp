...
ScreenSelectMusic::BeginScreen()
{
	...
	if (GAMESTATE->GetCurrentStyle(PLAYER_INVALID) == nullptr) {
		Locator::getLogger()->warn(
		  "The Style has not been set.  A theme must set the Style "
		...
		if (pStyle == nullptr) {
			Locator::getLogger()->warn(
			  ssprintf("No compatible styles for %s with %d player%s.",
...
ScreenSelectMusic::ChangeSteps(PlayerNumber pn, int dir)
{
	Locator::getLogger()->debug(
	  "ScreenSelectMusic::ChangeSteps( {}, {} )", pn, dir);
...
ScreenSelectMusic::SelectCurrent(PlayerNumber pn, GameplayMode mode)
{
	...
	switch (m_SelectionState) {
		case SelectionState_Finalized: {
			Locator::getLogger()->warn(
			  "song selection made while selectionstate_finalized");
			return false;
		}
