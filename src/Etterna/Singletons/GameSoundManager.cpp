...
GameSoundManager::StartQueuedSounds()
{
	...
	std::vector<std::string> aSoundsToPlayOnceFromDir =
	...
	if (PREFSMAN->m_bLogSkips && !g_Playing->m_bTimingDelayed) {
		...
		if (sLastFile == ThisFile && fSoundTimePassed >= 0 &&
			...
			Locator::getLogger()->trace(
			  "Song position skip in {}: expected {:.3f}, got {:.3f} (cur "
			  "{}, prev {}) ({:.3f} difference)",
...
{
	// This function is primarily just useful for hacking MP3 sync back to
	// normal since moving sound backwards force it to completely resync at the
	// moment
