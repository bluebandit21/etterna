 ...
 *
 * Windows is very particular about how a message is printed out. The built in
 * appenders use a function that do not function well with stdout/stderr
 * redirection as we do in ILogger.cpp This appender is specifically for windows
 * as it will work with redirection.
 */
...
class WindowsAppender : public IAppender
{
  public:
	...
	{
		util::nstring str =
		  Formatter::format(record); // Get the formatted print string
...
// Console Appender. One for windows, and another for other operating systems.
	...
	std::strftime(
	  timeString, sizeof(timeString), "%Y_%m_%d-%H_%M_%S", std::localtime(&t));
	...
	auto appDirPerms =
	  fs::status(Core::Platform::getAppDirectory()).permissions();
...
void
PlogLogger::setLogLevel(Core::ILogger::Severity logLevel)
{
	plog::get()->setMaxSeverity(convertSeverity(logLevel));
}

std::string
PlogLogger::getLogFile()
{
	return currentLogFile;
}

plog::Severity
PlogLogger::convertSeverity(ILogger::Severity logLevel)
{
	switch (logLevel) {
		case Severity::TRACE:
